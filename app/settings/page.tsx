    ...
    <div className="max-w-3xl space-y-6">
      ...
      <section className="card space-y-3 p-4">
        ...
        <ul className="space-y-2 text-sm">
          <Status
            ok={Boolean(env.COC_API_TOKEN)}
            label="Clash of Clans API token"
            hint="Set COC_API_TOKEN. Whitelist IP 45.79.218.79 when creating the key."
          />
          <Status
            ok={Boolean(env.DATABASE_URL)}
            label="Hosted database (Supabase)"
            hint="Not set — using the local embedded database in ./.data (fine for local use)."
          />
          <Status
            ok={Boolean(env.ADMIN_PASSWORD)}
            label="Admin password"
            hint="Set ADMIN_PASSWORD. Required when hosted."
          />
          <Status
            ok={Boolean(env.CRON_SECRET)}
            label="Daily auto-sync secret"
            hint="Set CRON_SECRET so the daily Vercel job can run."
          />
          <Status
        ...
        <div className="text-xs text-muted">
          API base: {env.COC_API_BASE || "https://cocproxy.royaleapi.dev/v1"}
        </div>
      ...
      <section className="card space-y-3 p-4">
        ...
        <p className="text-sm text-muted">
          ...
          <code className="text-text">
            {env.GOOGLE_SERVICE_ACCOUNT_EMAIL || "your service account email"}
          </code>
          , and paste its link here. Each export writes a tab named “CWL &lt;season&gt;”.
        </p>
        <SettingInput
          settingKey="exportSheetUrl"
          initial={row?.value ?? ""}
