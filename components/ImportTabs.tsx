    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1">
        {tabs.map((t) => (
          <button
            key={t.id}
            className={`btn ${tab === t.id ? "border-accent text-accent" : ""}`}
            onClick={() => setTab(t.id)}
          >
            {t.label}
          </button>
...
function HistoryImport() {
  ...
  const [cols, setCols] = useState<
    { index: number; on: boolean; seasonId: string; label: string; sortKey: string }[]
  >([]);
  ...
  const load = () =>
    exec(async () => {
      ...
      const id = Math.max(
        0,
        r.header.findIndex((h) => /^(discord )?id$/i.test(h.trim())),
      );
      setIdCol(id);
      ...
      setCols(
        r.header.map((h, index) => {
          ...
          return {
            index,
            on: Boolean(g) && index !== id && g!.id < current,
            seasonId,
            label: g ? seasonLabel(seasonId) : h.trim(),
            sortKey,
          };
        }),
      );
  ...
  const set = (i: number, patch: Partial<(typeof cols)[number]>) =>
    setCols(cols.map((c) => (c.index === i ? { ...c, ...patch } : c)));

  return (
    <div className="card space-y-3 p-4">
      <p className="text-muted">
        One row per member: a <b>Discord ID</b> column (or player tag starting with #) and one TRUE/FALSE
        column per season — like your <code>DB</code> tab. Re-importing a season replaces its imported marks.
        For two events in one month use ids like <code>2026-06</code> and <code>2026-06-2</code> with
                  ...
                  <tr key={c.index}>
                    <td className="td">
                      <input
                        type="checkbox"
                        checked={c.on}
                        onChange={(e) => set(c.index, { on: e.target.checked })}
                      />
                    </td>
                    ...
                    <td className="td">
                      <input
                        ...
                        value={c.seasonId}
                        onChange={(e) => set(c.index, { seasonId: e.target.value })}
                        placeholder="2026-05"
                      />
                    </td>
                    <td className="td">
                      <input
                        className="input w-32 py-1"
                        value={c.label}
                        onChange={(e) => set(c.index, { label: e.target.value })}
                      />
                    </td>
                    <td className="td">
                      <input
                        className="input w-32 py-1"
                        value={c.sortKey}
                        onChange={(e) => set(c.index, { sortKey: e.target.value })}
          ...
          <button
            ...
            onClick={() => {
              ...
              if (bad)
                return setResult({
                  ok: false,
                  message: `Column “${header[bad.index]}” needs a season id and an order date (YYYY-MM-DD).`,
                });
              exec(() =>
                doImportHistory(
                  src,
                  idCol,
                  chosen.map(({ index, seasonId, label, sortKey }) => ({ index, seasonId, label, sortKey })),
                ),
              );
            }}
          >
...
function DonationsImport({ defaultSeason }: { defaultSeason: string }) {
  ...
  return (
    <div className="card space-y-3 p-4">
      <p className="text-muted">
        Needs <b>Tag</b> and <b>Total Donated</b> columns (optional: Total Received, Name, Username, ID).
        Imported totals replace API snapshots for that season.
      </p>
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <label className="flex items-center gap-2 pb-2 text-sm">
          <input type="checkbox" checked={links} onChange={(e) => setLinks(e.target.checked)} /> Fill missing
          Discord links from Username / ID columns
        </label>
        <button
          className="btn btn-primary"
          disabled={pending || !src.trim()}
          onClick={() => exec(() => doImportDonations(src, season, links))}
        >
...
function CwlImport({
  ...
  return (
    <div className="card space-y-3 p-4">
      <p className="text-muted">
        Fallback when the API no longer has the war data. Needs <b>Tag</b> and <b>Number of Attacks</b>{" "}
        columns. Import one clan&apos;s export at a time.
      </p>
      <Source value={src} onChange={setSrc} />
      <div className="flex flex-wrap items-end gap-3">
        <div>
        ...
        <button
          className="btn btn-primary"
          disabled={pending || !src.trim() || !clan || !season}
          onClick={() => exec(() => doImportCwl(src, season, clan))}
        >
...
function PlayersImport() {
  ...
  return (
    <div className="card space-y-3 p-4">
      <p className="text-muted">
        Needs a <b>Tag</b> column. Optional: Name, ID (Discord ID), Username, PN (e.g. “PN2” or “2”), Guest
      ...
      <button
        className="btn btn-primary"
        disabled={pending || !src.trim()}
        onClick={() => exec(() => doImportPlayers(src))}
      >
